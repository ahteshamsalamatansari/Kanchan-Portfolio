...
var onePageLink = {
  ...
  render: function () {
    ...
    window.addEventListener(
      "scroll",
      debounce(this.handleWindowOnScrolled.bind(this), 300)
    );
  },
  ...
  setActiveMenuLink: function (sectionId) {
    const currentActiveLink = document.querySelector(
      `${this.config.linkSelector}.${this.config.currentClassName}`
    );
...
var BackToTop = {
  ...
  init: function () {
    ...
    window.addEventListener(
      "scroll",
      debounce(this.updateProgress.bind(this), 100)
    );
  },
